...
function addConnection(label, good) {
...
function refreshActiveCameraStream() {
    stopCameraStreams();
    if (socket?.readyState !== WebSocket.OPEN || !state.cameraReady)
        return;
    const target = activeView === "camera" ? cameraFeed : activeView === "pilot" ? pilotFeed : null;
    if (target)
        target.src = `${streamBase}?ts=${Date.now()}`;
}
function setLinkStatus(label, phase) {
...
function updateStateUI() {
    ...
    ["leftThrottle", "rightThrottle", "stopMotors"].forEach((id) => $(id).disabled = !motorEnabled);
    if (!motorEnabled && (motorInput.left !== 0 || motorInput.right !== 0))
        neutralizeMotors(false);
    const ballastEnabled = connected && state.pilot && state.calibrated;
    ["masterBallast", "frontTrim", "rearTrim", "surfaceButton", "diveButton", "emergencyButton"].forEach((id) => $(id).disabled = !ballastEnabled);
    ...
    $("frameSize").value = state.frameSize;
...
function send(type, payload = {}, log = true) {
    if (!socket || socket.readyState !== WebSocket.OPEN) {
        if (log)
            showToast("Controller is offline", true);
        return -1;
    }
    const seq = ++sequence;
    ...
    if (type !== "heartbeat")
        sentCommands++;
    if (log)
        addLog("control", type.replace(/_/g, " "), JSON.stringify(payload));
    return seq;
}
function sendActuator(type, payload, log = false) {
    if (isSimulator) {
    ...
    if (!pilotToken || socket?.readyState !== WebSocket.OPEN) {
        if (log)
            showToast("Claim pilot control first", true);
        return;
    }
    ...
    void fetch(`${httpBase}/set?${params}`, { cache: "no-store" })
        ...
        missedCommands++;
        if (log)
            showToast(error.message || "Control command failed", true);
    });
}
function connect() {
    ...
    current.addEventListener("message", (event) => { if (socket === current)
...
function handleMessage(text) {
    ...
    if (message.type === "ack") {
        if (typeof message.pilotToken === "number")
            pilotToken = message.pilotToken;
        if (typeof message.rtt === "number")
            latencyMs = Math.round(message.rtt);
        if (message.ok === false) {
            missedCommands++;
    ...
    if (message.type === "event") {
        ...
        if (event === "failsafe")
            showToast("Failsafe triggered: surfacing", true);
    }
...
function applyState(incoming) {
    if (typeof incoming.frontBallastDeg !== "number" && typeof incoming.frontBallast === "number")
        incoming.frontBallastDeg = Math.round((1 - incoming.frontBallast) * 180);
    if (typeof incoming.rearBallastDeg !== "number" && typeof incoming.rearBallast === "number")
        incoming.rearBallastDeg = Math.round((1 - incoming.rearBallast) * 180);
    const cameraReadinessChanged = typeof incoming.cameraReady === "boolean" && incoming.cameraReady !== state.cameraReady;
    state = { ...state, ...incoming };
    if (incoming.pilot === false)
        pilotToken = 0;
    if (cameraReadinessChanged)
        refreshActiveCameraStream();
    if ((!state.armed || state.failsafe) && (motorInput.left !== 0 || motorInput.right !== 0))
        neutralizeMotors(false);
    ...
    if (incoming.calibration && !calibrationLoaded) {
        ...
        numbers.forEach(([id, key]) => ($(id).value = String(calibration[key])));
        ["invertLeft", "invertRight", "invertFront", "invertRear"].forEach((key) => ($(key).checked = calibration[key]));
...
function renderMotorInputs() {
    ...
    const levels = directMotorLevels(motorInput.left, motorInput.right, Number($("throttleLimit").value) / 100);
...
function sendMotorCommand(log = false) {
    if (!state.armed)
        return;
    const limit = Number($("throttleLimit").value) / 100;
    sendActuator("motors", { left: motorInput.left, right: motorInput.right, limit }, log);
}
function queueMotorCommand() {
    renderMotorInputs();
    if (motorSendTimer)
        return;
...
function setMotorInput(side, value) {
    motorInput[side] = clamp(value, -1, 1);
    queueMotorCommand();
}
function releaseMotor(side) {
...
function neutralizeMotors(sendStop = true, log = false) {
    window.clearTimeout(motorSendTimer);
    motorSendTimer = 0;
    motorInput = { left: 0, right: 0 };
    renderMotorInputs();
    if (sendStop && state.armed)
        sendActuator("motors", { left: 0, right: 0, limit: 1 }, log);
}
function signedDegrees(value) {
    return `${value > 0 ? "+" : ""}${value}°`;
}
function currentBallastAngles() {
    const master = Number($("masterBallast").value);
    const frontTrim = Number($("frontTrim").value);
    const rearTrim = Number($("rearTrim").value);
    return { master, frontTrim, rearTrim, ...effectiveBallastAngles(master, frontTrim, rearTrim) };
}
function renderBallastControls() {
    ...
    ["masterBallast", "frontTrim", "rearTrim"].forEach((id) => setRangeFill($(id)));
...
function setBallastControls(master, frontTrim, rearTrim, sendCommand = true, mode = "") {
    $("masterBallast").value = String(Math.round(clamp(master, 0, 180)));
    $("frontTrim").value = String(Math.round(clamp(frontTrim, -30, 30)));
    $("rearTrim").value = String(Math.round(clamp(rearTrim, -30, 30)));
    renderBallastControls();
    if (sendCommand)
        sendBallastCommand(mode, true);
}
...
async function toggleRecording() {
    ...
    mediaRecorder.ondataavailable = (event) => { if (event.data.size)
...
function calibrationPayload() {
    const values = ["escMin", "escNeutral", "escMax", "frontSurface", "frontDive", "rearSurface", "rearDive"].map((id) => Number($(id).value));
    ...
    return {
        ...
        invertFront: $("invertFront").checked,
        invertRear: $("invertRear").checked,
    };
}
document.querySelectorAll("[data-view-target]").forEach((element) => element.addEventListener("click", () => showView(element.dataset.viewTarget)));
...
["left", "right"].forEach((side) => {
    ...
    input.addEventListener("lostpointercapture", () => { if (motorInput[side] !== 0)
        releaseMotor(side); });
    input.addEventListener("keyup", (event) => {
        if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Home", "End", "PageUp", "PageDown"].includes(event.key))
            releaseMotor(side);
    });
    input.addEventListener("blur", () => { if (motorInput[side] !== 0)
        releaseMotor(side); });
});
...
$("throttleLimit").addEventListener("input", () => { $("throttleOutput").textContent = `${$("throttleLimit").value}%`; queueMotorCommand(); });
["pilotLight", "cameraLight"].forEach((id) => $(id).addEventListener("input", () => setLight(Number($(id).value))));
...
$("overlayToggle").addEventListener("change", () => $("cameraOverlay").toggleAttribute("hidden", !($("overlayToggle").checked)));
$("frameSize").addEventListener("change", () => send("camera", { frameSize: $("frameSize").value, quality: Number($("jpegQuality").value) }));
$("jpegQuality").addEventListener("input", () => $("qualityOutput").textContent = $("jpegQuality").value);
$("jpegQuality").addEventListener("change", () => send("camera", { frameSize: $("frameSize").value, quality: Number($("jpegQuality").value) }));
...
$("clearLogs").addEventListener("click", () => { if (confirm("Clear all locally stored mission logs?")) {
    logs = [];
    localStorage.removeItem("subrc.logs.v1");
    renderLogs();
    showToast("Mission logs cleared");
} });
$("propsRemoved").addEventListener("change", () => $("saveCalibration").disabled = !($("propsRemoved").checked) || !state.pilot);
$("saveCalibration").addEventListener("click", () => { const payload = calibrationPayload(); if (payload)
    send("calibration", payload); });
$("saveFailsafe").addEventListener("click", () => send("config", { failsafeMs: Number($("failsafeTimeout").value) }));
$("saveWifi").addEventListener("click", () => { const password = $("apPassword").value; if (password.length < 8)
    return showToast("Wi-Fi password must contain at least 8 characters", true); if (confirm("Save the new Wi-Fi password and restart RC-01?"))
    send("config", { apPassword: password, restart: true }); });
$("exportConfig").addEventListener("click", () => downloadBlob(`submarine-config-${Date.now()}.json`, new Blob([JSON.stringify({ version: 1, failsafeMs: Number($("failsafeTimeout").value), frameSize: $("frameSize").value, jpegQuality: Number($("jpegQuality").value), calibration: calibrationPayload() }, null, 2)], { type: "application/json" })));
$("importConfig").addEventListener("change", async () => { const file = ($("importConfig").files ?? [])[0]; if (!file)
    return; const parsed = safeJson(await file.text()); if (!parsed || parsed.version !== 1)
    return showToast("Unsupported configuration file", true); send("config_import", { config: parsed }); });
window.addEventListener("beforeunload", () => { if (state.pilot) {
    neutralizeMotors();
    send("release", {}, false);
} stopHeartbeat(); });
window.addEventListener("blur", () => { if (state.armed)
    neutralizeMotors(); });
